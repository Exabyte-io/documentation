...
!!!warning "Restrictions on kgrid size"
    The user is advised that the default settings in the HSE band gap computation workflow are such that the q-grid is set to be half the size of the kgrid entered by the user. Hence an **even kgrid size** is always required, for example 8x8x8 yielding a q-grid of size 4x4x4. Should the user enter an odd number for the kgrid size by mistake, this size will automatically be increased by one in the workflow to make it even: for example, a 5x5x5 kgrid size entered by the user is increased to 6x6x6 by the workflow operations, thus resulting in a 3x3x3 q-grid. In order to change this default behaviour, the user is invited to manually edit the Quantum ESPRESSO input script for the main HSE calculation directly through the corresponding [unit editor interface](../../../workflow-designer/unit-editor.md), within its [input script template](../../../workflow-designer/unit-editor/input-templates.md).
