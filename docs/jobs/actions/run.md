...
A Job listed under the [Jobs Explorer](../ui/explorer.md) which has a ["pre-submission" status](../status.md) can be submitted to the queue of the cluster for execution of its computational tasks by "Running" it. 
...
In the example below, we demonstrate how the status of a Job changes as described above when it is Run.
