...
# ALL entered keywords have to be contained within pathname of file of interest, and point to its exact location
...
for i in range(len(list_files)):
    ...
    if [list_path[j] in list_keywords  for j in range(len(list_path))].count(True) >= len(list_keywords):
        common_file = open(list_files[i])
        list_lines = common_file.readlines()
