...
First of all, in order to perform material simulation, we need to specify the
material structure. We can create a material on our platform using
...
calculation. Alternatively, you may find required workflow in the workflow bank
in our platform, and can import to your library/account. Our example calculation
...
in conjunction to spin-polarization, please select the respective template.

### 2.2. Bands calculation

In the next step, we add a unit for bands calculation and select **pw_bands_magn**
template.

### 2.3. Bands.x postprocessing

In the finals step, we add `bands.x` calculation. We are interested in
...
outputs written to different files for up/down spins. It is recommended to give
each units a distinct names, otherwise some of the generated files mights be
overwritten.

## 3. Job designer

Finally, navigate to jobs page and click create new job. Import the material and
workflow. Then navigate to the **Important Settings** tab under workflow. Here
we can set the `starting_magnetization`. Since, we want to perform
...
the bands calculation. We will set the `starting_magnetization` same as above
step. However, note that `starting_magnetization` may not be used in case
...
to set the `starting_magnetization` same as `scf` step.

If necessary, adjust the compute parameters in the **compute** tab. Finally, we
are ready for job submission.
